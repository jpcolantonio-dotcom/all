...
const RESTAURANTES = [
  { id: null, nombre: 'El Bodegón NKT', emoji: '🦞', cat: 'Mariscos · Pizzas · Sándwiches' },
  { id: null, nombre: "Colantonio's", emoji: '🍕', cat: 'Pizza napolitana · Empanadas' },
  { id: null, nombre: 'Kusama Sushi', emoji: '🍣', cat: 'Sushi · Mariscos' },
]
    ...
    <div style={s.bg}>
      <div style={s.loginCard}>
        ...
        <div style={s.loginBody}>
          ...
          <button onClick={ingresar} style={s.btnMain}>Ingresar →</button>
        </div>
      </div>
    </div>
  )
        ...
        <div style={s.scroll}>
          {tabActiva === 'pedidos' && (
            <>
              ...
              <div style={s.statsRow}>
                <div style={{...s.stat, ...s.statHighlight}}><div style={s.statVal}>3</div><div style={s.statLabel}>Nuevos</div></div>
              ...
              {[
                ...
                <div key={i} style={s.pedidoCard}>
                  ...
                  <div style={s.pedidoItems}>
                    {p.items.map((it,j) => <div key={j} style={s.itemLine}>{it}</div>)}
                  </div>
                  <div style={s.pedidoFooter}>
                    <div>
                      <div style={s.pedidoTotal}>{p.total}</div>
                      <div style={s.pedidoDir}>📍 {p.dir}</div>
                    </div>
          ...
          {tabActiva === 'menu' && (
            <>
              ...
              {[
                ...
                <div key={i} style={s.menuItem}>
                  <div style={{flex:1}}>
                    ...
                    <div style={s.menuDesc}>{prod.desc}</div>
                  </div>
              ...
              <div style={s.catLabel}>PIZZAS</div>
              {[
                {nombre:'Pizza de mariscos', desc:'Mozzarella, langostinos, rúcula', precio:'$5.900'},
                {nombre:'Pizza muzarella', desc:'Clásica con tomate y mozzarella', precio:'$4.100'},
              ].map((prod, i) => (
                <div key={i} style={s.menuItem}>
                  <div style={{flex:1}}>
                    <div style={s.menuNombre}>{prod.nombre}</div>
          ...
          {tabActiva === 'local' && (
            <>
              ...
              <div style={s.infoCard}>
                <div style={s.infoLabel}>Tiempo de entrega</div>
                <div style={s.infoVal}>25 – 40 min</div>
              </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  restEmoji: {fontSize:22, width:40, height:40, borderRadius:10, background:'#fff', display:'flex', alignItems:'center', justifyContent:'center', border:'1px solid #eee'},
  ...
  topbarName: {fontSize:13, fontWeight:700, color:'#1A1A1A', letterSpacing:-0.3},
  topbarSub: {fontSize:10, color:'#888', marginTop:1},
  estadoBadge: {marginLeft:'auto', display:'flex', alignItems:'center', gap:5},
  ...
  toggleTrack: {width:42, height:24, borderRadius:20, position:'relative' as const, cursor:'pointer', transition:'.2s', flexShrink:0},
  ...
  statHighlight: {background:'#FF4D00'},
  ...
  pedidoHead: {padding:'10px 12px', display:'flex', alignItems:'center', gap:8, borderBottom:'1px solid #FFE8DC'},
  ...
  itemLine: {fontSize:11, color:'#555', marginBottom:3},
  ...
  pedidoDir: {fontSize:10, color:'#888'},
  ...
  menuDesc: {fontSize:10, color:'#888', marginTop:1},
